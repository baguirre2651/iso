...
import { ExtensionUninstallEvent, ExtensionDisableEvent, ExtensionEnableEvent, KeychainTokenStorage, debugLogger, } from '@google/gemini-cli-core';
...
describe('extension tests', () => {
    ...
    describe('loadExtensions', () => {
        ...
        it('should skip extensions with invalid JSON and log a warning', async () => {
            ...
            expect(debugErrorSpy).toHaveBeenCalledExactlyOnceWith(expect.stringContaining(`Warning: Skipping extension in ${badExtDir}: Failed to load extension config from ${badConfigPath}`));
        ...
        it('should skip extensions with missing name and log a warning', async () => {
            ...
            expect(extensions).toHaveLength(1);
            expect(extensions[0].name).toBe('good-ext');
            expect(debugErrorSpy).toHaveBeenCalledExactlyOnceWith(expect.stringContaining(`Warning: Skipping extension in ${badExtDir}: Failed to load extension config from ${badConfigPath}: Invalid configuration in ${badConfigPath}: missing "name"`));
            debugErrorSpy.mockRestore();
        });
